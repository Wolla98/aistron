...
__all__ = ["load_aistron_cocolike_json", "register_aistron_cocolike_instances"]


def load_aistron_cocolike_json(json_file, image_root, dataset_name=None, extra_annotation_keys=None):
    """
...
"""
    ...
    for (img_dict, anno_dict_list) in imgs_anns:
        ...
        for anno in anno_dict_list:
            ...
            amodal_segm = anno.get("amodal_segm", None)
...
def register_aistron_cocolike_instances(name, metadata, json_file, image_root):
    """
    ...
    DatasetCatalog.register(name, lambda: load_aistron_cocolike_json(json_file, image_root, name))

    # 2. Optionally, add metadata about this dataset,
    # since they might be useful in evaluation, visualization or logging
    MetadataCatalog.get(name).set(
        json_file=json_file, image_root=image_root, evaluator_type="coco_amodal", **metadata
    )
